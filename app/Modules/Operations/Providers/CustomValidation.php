...
class CustomValidation extends Validator
{
     ...
     * $value Input value
     * $parameters Table, field1, field2, value field2
     */
    public function validateUniqueWith($attribute, $value, $parameters)
    {
        ...
        $result = DB::table($parameters[0])->where(function ($query) use ($attribute, $value, $parameters) {
            $query->where($attribute, '=', $value) // fecha = value
                ->where($parameters[1], '=', $parameters[2]); // where currency_id = $parameters[2]
        })->first();
    ...
    public function validateUniqueMultiple($attribute, $value, $parameters)
    {
        // Now that we have our data we can check for the data
        ...
        $result = DB::table($parameters[0])->where(function ($query) use ($attribute, $value, $parameters) {
            $query->where($attribute, '=', $value) // fecha = value
                ...
                ->where($parameters[3], '=', $parameters[4]); // where modelterminal = $parameters[4]
        })->first();
