...
use App\Modules\Parameters\Models\Comission;
use Auth;
...
class ComissionRepository implements ComissionInterface
{
    ...
    /**
     * Comission Repository constructor.
     *
    ...
    public function comissionArray($request)
    {
    }
    ...
    public function api()
    {
        $raw = \DB::raw("*, CONCAT(comissions.description, ' - ',comissions.type_condition) as fullname");

        return $this->model->select($raw)
            ->whereNull('comissions.deleted_at')
            ->pluck('fullname', 'comissions.id');
    }
}
