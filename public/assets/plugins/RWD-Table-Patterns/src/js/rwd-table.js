(function ($) {
    ...
    ResponsiveTable.prototype.createButtonToolbar = function () {
        ...
        if (this.options.addFocusBtn) {
            ...
            // Create focus btn
            this.$focusBtn = $(
                '<button class="btn btn-default">Focus</button>'
            );
    ...
    ResponsiveTable.prototype.updateStickyTableHeader = function () {
        ...
        var shouldBeVisible =
            scrollTop > offsetTop &&
            scrollTop < offsetTop + that.$table.height();

        if (useFixedSolution) {
            that.$stickyTableHeader.scrollLeft(
                that.$tableScrollWrapper.scrollLeft()
            );
