(function (window) {
    ...
    c3_chart_internal_fn.initWithData = function (data) {
        ...
        main.insert(
            "rect",
            config.zoom_privileged ? null : "g." + CLASS.regions
        )
    ...
    c3_chart_internal_fn.redraw = function (options, transitions) {
        ...
        transitions =
            transitions || $$.axis.generateTransitions(durationForAxis);
    ...
    c3_chart_internal_fn.transformMain = function (
        withTransition,
        transitions
    ) {
        var $$ = this,
    ...
    c3_chart_internal_fn.convertUrlToData = function (
        url,
        mimeType,
        keys,
        done
    ...
    c3_chart_internal_fn.redrawEventRect = function () {
        ...
        if (isMultipleX) {
            ...
            eventRects.datum(
                maxDataCountTarget ? maxDataCountTarget.values : []
            );
            $$.eventRect = eventRects.selectAll("." + CLASS.eventRect);
    ...
    c3_chart_internal_fn.generateEventRectsForSingleX = function (
        ...
        eventRectEnter
            ...
            .on("mousemove", function (d) {
                var selectedData,
                    index = d.index,
                    eventRect = $$.svg.select(
                        "." + CLASS.eventRect + "-" + index
                    );
                ...
                // Show tooltip
                selectedData = $$.filterTargetsToShow($$.data.targets).map(
                    function (t) {
                ...
                if ($$.isBarType(closest.id) || $$.dist(closest, mouse) < 100) {
                    $$.main
                        .selectAll(
                            "." +
                                CLASS.shapes +
                                $$.getTargetSelectorSuffix(closest.id)
                        )
    ...
    c3_chart_internal_fn.dispatchEvent = function (type, index, mouse) {
        var $$ = this,
            selector =
                "." + CLASS.eventRect + (!$$.isMultipleX() ? "-" + index : ""),
    ...
    c3_chart_internal_fn.getAxisWidthByAxisId = function (
        id,
        withoutRecompute
    ) {
        var $$ = this,
        ...
        if (
            ...
            h =
                ...
                $$.axis.getMaxTickWidth(axisId) *
                    Math.cos(
                        (Math.PI * (90 - config.axis_x_tick_rotate)) / 180
                    );
        }
        return (
            h +
    ...
    c3_chart_internal_fn.getShapeOffset = function (
        typeFilter,
        indices,
        isSub
    ) {
    ...
    c3_chart_internal_fn.updateXGrid = function (withoutUpdate) {
        ...
        if (!withoutUpdate) {
            $$.xgrid.attr($$.xgridAttr).style("opacity", function () {
                return +d3
                    .select(this)
                    .attr(config.axis_rotated ? "y1" : "x1") ===
    ...
    c3_chart_internal_fn.redrawArc = function (
        ...
        main.select("." + CLASS.chartArcsTitle).style(
            "opacity",
            $$.hasType("donut") || $$.hasType("gauge") ? 1 : 0
        );

        if ($$.hasType("gauge")) {
            $$.arcs
                .select("." + CLASS.chartArcsBackground)
                .attr("d", function () {
                    var d = {
            ...
            $$.arcs
                ...
                .attr(
                    "dx",
                    -1 * ($$.innerRadius + ($$.radius - $$.innerRadius) / 2) +
                        "px"
                )
                .attr("dy", "1.2em")
            ...
            $$.arcs
                ...
                .attr(
                    "dx",
                    $$.innerRadius + ($$.radius - $$.innerRadius) / 2 + "px"
                )
                .attr("dy", "1.2em")
    ...
    c3_chart_internal_fn.initZoom = function () {
        ...
        $$.zoom.updateScaleExtent = function () {
            var ratio =
                    diffDomain($$.x.orgDomain()) / diffDomain($$.orgXDomain),
    ...
    c3_chart_internal_fn.generateClass = function (prefix, targetId) {
        return (
            " " + prefix + " " + prefix + this.getTargetSelectorSuffix(targetId)
        );
    };
        ...
        return function () {
            ...
            d3.transition()
                ...
                .each(function () {
                    ...
                    wait.add(
                        mainLine.transition().attr("transform", transform)
                    );
                    wait.add(
                        mainArea.transition().attr("transform", transform)
                    );
                    wait.add(
                        mainCircle.transition().attr("transform", transform)
                    );
                    wait.add(
                        mainText.transition().attr("transform", transform)
                    );
                    wait.add(
                        mainRegion
                            .filter($$.isRegionOnX)
                    ...
                    wait.add(
                        xgridLines.transition().attr("transform", transform)
                    );
                })
                .call(wait, function () {
                    var i,
                    ...
                    if (flowLength) {
                        for (i = 0; i < flowLength; i++) {
                            shapes.push(
                                "." + CLASS.shape + "-" + (flowIndex + i)
                            );
                            texts.push(
                                "." + CLASS.text + "-" + (flowIndex + i)
                            );
                            eventRects.push(
                                "." + CLASS.eventRect + "-" + (flowIndex + i)
                            );
                        }
                        $$.svg
    ...
    function c3_axis(d3, params) {
        ...
        function axis(g) {
            g.each(function () {
                ...
                function splitTickText(d, maxWidth) {
                    ...
                    if (
                        Object.prototype.toString.call(tickText) ===
                        "[object Array]"
                    ) {
                        return tickText;
                    }
                    ...
                    if (!maxWidth || maxWidth <= 0) {
                        maxWidth = isVertical
                            ? 95
                            : params.isCategory
                            ? Math.ceil(scale1(ticks[1]) - scale1(ticks[0])) -
                              12
                            : 110;
                    }
                ...
                function tspanDy(d, i) {
                    ...
                    if (i === 0) {
                        if (orient === "left" || orient === "right") {
                            dy = -(
                                (counts[d.index] - 1) * (sizeFor1Char.h / 2) -
                                3
                            );
                        } else {
                            dy = ".71em";
                ...
                function tickSize(d) {
                    var tickPosition =
                        scale(d) + (tickCentered ? 0 : tickOffset);
        ...
        axis.orient = function (x) {
            ...
            orient =
                x in { top: 1, right: 1, bottom: 1, left: 1 }
                    ? x + ""
                    : "bottom";
            return axis;
        };
